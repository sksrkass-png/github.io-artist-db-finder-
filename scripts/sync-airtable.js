...
function extractUrlsFromText(value) {
  ...
  const urls = [];
  for (const match of text.matchAll(/\((https?:\/\/[^)]+)\)/g)) urls.push(match[1]);
...
function boolValue(value) {
  ...
  return ["true", "yes", "y", "추천", "1", "checked", "o", "ok"].includes(String(value).trim().toLowerCase());
}
...
function mapRecord(record) {
  ...
  const artworkImages = [...separateImageUrls, ...attachmentImageUrls].filter(Boolean).filter((url, idx, arr) => arr.indexOf(url) === idx).slice(0, 3);
  ...
  const artist = {
    ...
    phone: getField(f, ["연락처", "phone", "Phone"], ""),
